...
module.exports = (transport) => {
  describe('.query()', () => {
    ...
    beforeAll(() => transport.log(info));

    it('should return matching results using default querying', async () => {
      const callbackMock = jest.fn();
      await transport.query({}, callbackMock);
    ...
    it('should return one result using the `rows` option', async () => {
      const callbackMock = jest.fn();
      await transport.log(info, async () => {
        await transport.query({ rows: 1 }, callbackMock);
        expect(callbackMock.mock.calls[0][0]).toBe(null);
    ...
    it('should return matching results using the `from` and `until` option', async () => {
      ...
      await transport.log(info, async () => {
        ...
        await transport.query({ from, until }, callbackMock);
    ...
    it('should return no results using a bad `from` and `until` option', async () => {
      ...
      await transport.log(info, async () => {
        ...
        await transport.query({ from: bad, until: bad }, callbackMock);
